...
CATEGORY_ORDER = ["mapsmanager", "localizer", "planner", "controller", "costmap_filters", "navmap_filters"]


def _ament_index_roots() -> List[str]:
    """Return all candidate '.../share/ament_index/resource_index' roots across overlay + system."""
    roots: List[str] = []
    ...
    for p in prefixes:
        ...
        if os.path.isdir(per_pkg) and per_pkg not in seen:
            roots.append(per_pkg); seen.add(per_pkg)
        ...
        if os.path.isdir(common) and common not in seen:
            roots.append(common); seen.add(common)
    return roots
...
class PluginsVerb(VerbExtension):
    ...
    def add_arguments(self, parser, cli_name):
        # Category filters
        parser.add_argument("--mapsmanager", action="store_true", help="Only show mapsmanager plugins.")
        parser.add_argument("--localizer", action="store_true", help="Only show localizer plugins.")
        parser.add_argument("--planner", action="store_true", help="Only show planner plugins.")
        parser.add_argument("--controller", action="store_true", help="Only show controller plugins.")
        parser.add_argument("--costmap-filters", action="store_true", help="Only show costmap filters.")
        parser.add_argument("--navmap-filters", action="store_true", help="Only show navmap filters.")

        # Output options
        parser.add_argument("--show-xml", action="store_true", help="Show the XML path declaring the plugin.")
        parser.add_argument("--show-lib", action="store_true", help="Show the <library> path if available.")
        parser.add_argument("--grep", help="Filter by substring in plugin name or type.")

        # Machine-readable + debug
        parser.add_argument("--json", action="store_true", help="Output JSON instead of text.")
        parser.add_argument("--pretty", action="store_true", help="Pretty-print JSON (only with --json).")
        parser.add_argument("--debug", action="store_true", help="Print debug info about ament index scanning.")
    ...
    def main(self, *, args):
        ...
        for key in CATEGORY_ORDER:
            ...
            json_out[key] = {"title": cfg["title"], "base": base, "count": len(plugins), "plugins": plugins}
            total += len(plugins)
